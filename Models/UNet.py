...
class UNet(nn.Module):
    ...
    @staticmethod
    def trainModel(dataset, data_type='Voice', n_epochs=20, batch_size=16, learning_rate=0.0001, valid_dataset=None, schedule = False):
        """
        ...
        for epoch in range(last_saved_epoch, last_saved_epoch + n_epochs):
            ...
            for X, Y in tqdm(dataloader):
                ...
                loss = criterion(output * X.unsqueeze(1),Y.unsqueeze(1))
                loss.backward()
                optimizer.step()
            ...
            if valid_dataloader:
                ...
                with torch.no_grad():
                    for X, Y in valid_dataloader:
                        ...
                        loss = criterion(output * X.unsqueeze(1),Y.unsqueeze(1))  # Add channel dimension for target
    ...
    @staticmethod
    def validateModel(voice_model, noise_model, valid_dataset, subset_size=50):
        """
        ...
        with torch.no_grad():
            for snr, sampled_indices in sampled_indices_by_snr.items():
                ...
                for dataset_idx in tqdm(sampled_indices, desc=f"Validation Progress (SNR={snr})"):
                    ...
                    voice_pred = (voice_model(X_) * X_).squeeze()
        ...
        for snr, metrics in metrics_by_snr.items():
            metrics['voice'].to_csv(os.path.join(SCORES_PATH, f'val_metrics_SNR_{snr}_voice.csv'), index=False)
            metrics['noise'].to_csv(os.path.join(SCORES_PATH, f'val_metrics_SNR_{snr}_noise.csv'), index=False)
